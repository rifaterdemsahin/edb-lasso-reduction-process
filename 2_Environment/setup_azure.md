...
### 2. Designated Key Vault (Mandatory)
> **MANDATE:** Use existing Key Vault: `/vaults/dp-kv-deliverypilot/secrets` (Vault name: `dp-kv-deliverypilot`). **Do NOT create a new Key Vault.** Save and load all credentials there (Fly.io, Cloudflare Workers, Azure Storage, APIs).

### 3. Registering Secrets
```bash
# Add a secret to the designated vault (/vaults/dp-kv-deliverypilot/secrets)
az keyvault secret set --vault-name dp-kv-deliverypilot --name "DATABASE-URL" --value "postgres://..."
```
