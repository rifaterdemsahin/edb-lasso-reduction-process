...
### SPEC-014: EDB Lasso Diagnostic Reduction Engine & Dual Manifests
- **Status:** Active
- **Description:** Diagnostic redaction pipeline for PostgreSQL/EDB Lasso support bundles, refactored onto the delivery-pilot-template structure (RULE-001 through RULE-005). Automatically sanitizes credentials, pseudonymizes IPs deterministically, recurses nested archives, preserves binary dumps, and generates dual manifests (`security_donotshare` for internal forensics and `support` with zero credentials for vendor tickets).
- **Key Behaviors:**
  - Core redaction engine resides at `5_Symbols/lasso_redact.py` with zero-pip standard-library architecture.
  - Multi-stage diagnostic pipeline: Stage 1 (Ingestion & Unpack), Stage 2 (Credential Stripping), Stage 3 (Deterministic IP Mapping), Stage 4 (Dual Manifest & Repack).
  - Repacked tarballs contain exclusively the clean support manifest; `security_donotshare` is placed strictly outside the bundle for internal SecOps compliance.
  - Full test suite resides at `7_Testing_Known/test_lasso_redact.py` (9 tests, all passing).
  - UI pages, interactive sandbox, and comparison visualizer reside in `5_Symbols/` and root `index.html`.
  - Presentation artifacts and mock bundle reside in `3_Simulation/`.
  - Secrets managed via Key Vault `/vaults/dp-kv-deliverypilot/secrets`.
- **Related Files:** `5_Symbols/lasso_redact.py`, `7_Testing_Known/test_lasso_redact.py`, `3_Simulation/presentation.md`, `5_Symbols/stage-04-audit-manifest-repack.html`, `index.html`
